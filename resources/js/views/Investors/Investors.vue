    ...
    <div>
        <div class="page-heading">
            ...
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>{{ __('manage_investors') }}</h3>
            </div>
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><router-link to="/dashboard">{{ __('dashboard') }}</router-link></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ __('manage_investors') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <span class="pull-right">
                                ...
                                <router-link to="/investors/create" class="btn btn-primary" v-b-tooltip.hover  title="Add New Seller" v-if="$can('seller_create')">{{ __('add_seller') }}</router-link>
                            </span>
                        </div>
                        <div class="card-body">
...
export default {
    ...
    methods: {
        getRecords(){
            this.isLoading = true;
            //axios.get(this.$apiUrl + '/investors/'+this.filterStatus)
            axios.get(this.$apiUrl + '/investors', {
                params: {
        ...
        deleteSeller(index, id){
            this.$swal.fire({
                ...
                if (result.value) {
                    ...
                    axios.post(this.$apiUrl + '/investors/delete',postData)
                        .then((response) => {
                            this.isLoading = false
                            let data = response.data;
        ...
        updateSellerCommission(){
            this.$swal.fire({
                ...
                if (result.value) {
                    this.isLoading = true
                    axios.get(this.$apiUrl + '/investors/updateCommission')
                        .then((response) => {
                            let data = response.data;
                            if (data.status === 1) {
                                //this.showSuccess(data.message)
