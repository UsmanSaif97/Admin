...
//dd($sellerIds);
    ...
    public static function sendNotificationOrderAssignDeliveryBoy($order){
        ...
        if (isset($order->delivery_boy_id) && $order->delivery_boy_id != 0 && $order->delivery_boy_id != "") {
           ...
           // if (self::checkOrderMailSendable($deliveryBoy->id, $order->active_status, 1)) {
                ...
                $userTokens = UserToken::where('user_id',$order->delivery_boy_id)->where('type','Delivery Boy')->get()->pluck('fcm_token','platform')->toArray();
                $message = "";
                $type="assign_order";
                self::sendNotification($userTokens,$title,$message,$type);
            //}
    ...
    public static function sendOrderItemStatusMailNotification($order_item, $type){
        ...
        try {
                   ...
                   dispatch(new SendEmailJob($order_item,'order_item_status_update'))->afterResponse();
    ...
    {
        return Investor::all(); // Retrieve all investors
    }

    public static function createInvestor($data)
    {
        return Investor::create($data); // Create a new investor
    }

    public static function updateInvestor($id, $data)
    {
        $investor = Investor::findOrFail($id);
        $investor->update($data);
        return $investor; // Update and return the investor
    }
