...
class CustomerAuthController extends Controller
{
    public function login(Request $request)
    {
        ...
        $user = User::select('auth_uid','id','name','email','country_code','mobile','profile','balance','referral_code','status','type')
                ->where('auth_uid', request()->auth_uid)->first();
        ...
        if ($requestData['type'] === 'phone') {
            $validator->after(function ($validator) use ($requestData) {
                if (empty($requestData['mobile'])) {
                    $validator->errors()->add('mobile', 'The mobile field is required when type is phone.');
                }
            });
        } elseif ($requestData['type'] === 'email') {
            $validator->after(function ($validator) use ($requestData) {
                if (empty($requestData['name'])) {
                    $validator->errors()->add('name', 'The name field is required when type is email.');
                }
                if (empty($requestData['email'])) {
                    $validator->errors()->add('email', 'The email field is required when type is email.');
                }
                if (empty($requestData['password'])) {
                    $validator->errors()->add('password', 'The password field is required when type is email.');
                }
            });
        }
        if ($validator->fails()) {
